...
use super::{fixed_options, fixed_options_with, register_json, supplier_options, QUEUE_ANALYTICS};
...
pub fn register(app: &mut Horsies) -> Result<(), HorsiesError> {
    for name in [
        ...
        register_json(app, name, QUEUE_ANALYTICS, options)?;
    }
    register_json(
        app,
        "sync_supplier_feed",
    ...
    register_json(app, "update_stock_levels", QUEUE_ANALYTICS, fixed_options())?;
    register_json(app, "prewarm_search", QUEUE_ANALYTICS, fixed_options())?;
    register_json(
        app,
        "warm_cache_edge",
    ...
    register_json(app, "update_price", QUEUE_ANALYTICS, fixed_options())?;
    Ok(())
}
