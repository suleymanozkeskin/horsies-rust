...
/// Typed reference to a node within a `WorkflowSpecBuilder`.
///
/// This is the public workflow-authoring reference type. It preserves the
/// source node's output type so typed data-flow APIs like `arg_from(...)`
/// can validate compatibility at compile time.
...
impl<T> TypedNodeRef<T> {
    ...
    /// Erase the output type and return the raw node reference.
    pub fn untyped(self) -> NodeRef {
        NodeRef { index: self.index }
    }
}
...
impl<T> From<TypedNodeRef<T>> for NodeRef {
    fn from(value: TypedNodeRef<T>) -> Self {
        NodeRef { index: value.index }
    }
}
...
/// Typed token for an input field on a workflow task or sub-workflow.
///
/// Generated by `#[derive(horsies::WorkflowInput)]` on named-field input
/// structs. Carries the owning input type and the field type, while lowering
/// to the raw JSON key at spec build time.
#[derive(Debug, PartialEq, Eq)]
pub struct InputField<I, F> {
    name: &'static str,
    _phantom: PhantomData<fn() -> (I, F)>,
}
...
impl<I, F> InputField<I, F> {
    ...
    /// The serialized field name used in kwargs JSON.
...
impl<I, F> Copy for InputField<I, F> {}
...
pub struct TaskNode<T, I = ()> {
    ...
    _phantom: PhantomData<fn() -> (T, I)>,
}

impl<T, I> std::fmt::Debug for TaskNode<T, I> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
...
impl<T, I> TaskNode<T, I> {
    ...
    /// Inject a dependency's `TaskResult<S>` as a typed input field.
    ///
    /// The dependency is also added to `waits_for` automatically.
    pub fn arg_from<S>(
        mut self,
        field: InputField<I, TaskResult<S>>,
        dep: TypedNodeRef<S>,
    ) -> Self {
...
mod tests {
    ...
    fn task_node_args_from_adds_dependency() {
        ...
        let node: TaskNode<()> = TaskNode::raw("t").raw_arg_from("raw_data", dep);
