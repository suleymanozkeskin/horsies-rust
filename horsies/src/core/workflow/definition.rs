...
/// Declarative macro for defining a workflow in a single block.
///
/// Generates a struct that implements [`WorkflowDefinition`] and provides
/// a `build()` method returning a validated `WorkflowSpec`.
///
/// # Syntax
///
/// ```ignore
...
///     name: "my_pipeline",
///     on_error: OnError::Pause,  // optional, defaults to Fail
///     nodes: |builder| {
///         let fetch = builder.task(TaskNode::<String>::new("fetch_data").node_id("fetch"));
///         let process = builder.task(
///             TaskNode::<Vec<u8>>::new("process_data")
///                 .args_from("raw", fetch)
///                 .node_id("process"),
///         );
///         let store = builder.task(
///             TaskNode::<()>::new("store_result")
///                 .args_from("data", process)
///                 .node_id("store"),
///         );
///         store  // last expression is the output node ref
///     },
/// }
/// ```
///
/// The macro expands to a function that builds a `WorkflowSpec` using
/// `WorkflowSpecBuilder`. The closure receives the builder and must return
/// a `NodeRef` (used as the output node) or `()` (no output).
///
/// # Examples
///
/// ## With output
...
///     name: "etl_pipeline",
///     nodes: |builder| {
///         let extract = builder.task(TaskNode::<String>::new("extract").node_id("extract"));
///         let transform = builder.task(
///             TaskNode::<String>::new("transform")
///                 .args_from("input", extract)
///                 .node_id("transform"),
///         );
///         let load = builder.task(
///             TaskNode::<()>::new("load")
///                 .args_from("data", transform)
///                 .node_id("load"),
///         );
///         load
///     },
...
///     nodes: |builder| {
///         let root = builder.task(TaskNode::<()>::new("dispatch").node_id("dispatch"));
///         builder.task(TaskNode::<()>::new("worker_a").waits_for(root).node_id("worker_a"));
///         builder.task(TaskNode::<()>::new("worker_b").waits_for(root).node_id("worker_b"));
///     },
/// };
/// ```
#[macro_export]
macro_rules! workflow {
    ...
    (
        ...
        nodes: |$builder:ident| $body:block $(,)?
    ) => {{
        let mut $builder = $crate::core::WorkflowSpecBuilder::new($name);
        $builder.on_error($on_error);
        let __output: $crate::core::NodeRef = $body;
        $builder.output(__output);
        $builder.build()
    }};

    // Variant: without on_error, nodes closure returns NodeRef (has output)
    (
        name: $name:expr,
        nodes: |$builder:ident| $body:block $(,)?
    ) => {{
    ...
    // Variant: with on_error, nodes closure returns () (no output)
    (
        name: $name:expr,
        on_error: $on_error:expr,
        nodes: |$builder:ident| $body:block,
    ...
    // Variant: without on_error, nodes closure returns () (no output)
    (
        name: $name:expr,
        nodes: |$builder:ident| $body:block,
        no_output $(,)?
    ...
    // Variant: with success_policy and on_error
    (
        name: $name:expr,
        on_error: $on_error:expr,
        success_policy: $policy:expr,
        nodes: |$builder:ident| $body:block $(,)?
    ) => {{
        let mut $builder = $crate::core::WorkflowSpecBuilder::new($name);
        $builder.on_error($on_error);
    ...
    // Variant: with success_policy, no on_error
    (
        name: $name:expr,
        success_policy: $policy:expr,
        nodes: |$builder:ident| $body:block $(,)?
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // workflow! macro tests
    // -----------------------------------------------------------------------

    #[test]
    fn macro_with_output() {
        let spec = workflow! {
            name: "macro_pipeline",
            nodes: |builder| {
                let a = builder.task(TaskNode::<()>::new("step_a").node_id("step_a"));
                builder.task(
                    TaskNode::<()>::new("step_b")
                        .waits_for(a)
                        .node_id("step_b"),
        ...
        assert_eq!(spec.name, "macro_pipeline");
    ...
    #[test]
    fn macro_with_on_error() {
        let spec = workflow! {
            name: "macro_pause",
            on_error: OnError::Pause,
            nodes: |builder| {
                builder.task(TaskNode::<()>::new("a").node_id("a"))
            },
        };
        let spec = spec.unwrap();
        assert_eq!(spec.on_error, OnError::Pause);
    ...
    fn macro_no_output() {
        let spec = workflow! {
            name: "macro_no_output",
            nodes: |builder| {
                let root = builder.task(TaskNode::<()>::new("root").node_id("root"));
                builder.task(TaskNode::<()>::new("a").waits_for(root).node_id("a"));
                builder.task(TaskNode::<()>::new("b").waits_for(root).node_id("b"));
            },
            no_output,
        };
        let spec = spec.unwrap();
        assert_eq!(spec.name, "macro_no_output");
        assert_eq!(spec.tasks.len(), 3);
        assert!(spec.output_index.is_none());
    }

    #[test]
    fn macro_no_output_with_on_error() {
        let spec = workflow! {
            name: "macro_no_out_pause",
            on_error: OnError::Pause,
            nodes: |builder| {
                builder.task(TaskNode::<()>::new("solo").node_id("solo"));
            },
            no_output,
        };
        let spec = spec.unwrap();
    ...
    fn macro_with_success_policy() {
        let spec = workflow! {
            ...
            nodes: |builder| {
                let a = builder.task(TaskNode::<()>::new("a").node_id("a"));
                builder.task(TaskNode::<()>::new("b").node_id("b"));
                a
            },
        };
        let spec = spec.unwrap();
        assert!(spec.success_policy.is_some());
        let policy = spec.success_policy.unwrap();
        assert_eq!(policy.cases[0].required_indices, vec![0]);
        assert_eq!(policy.optional_indices, Some(vec![1]));
    }

    #[test]
    fn macro_with_on_error_and_success_policy() {
        let spec = workflow! {
            name: "macro_full",
            on_error: OnError::Pause,
            success_policy: SuccessPolicy {
    ...
    #[test]
    fn macro_validation_errors_propagate() {
        let result = workflow! {
            name: "",
            nodes: |builder| {
                builder.task(TaskNode::<()>::new("a"))
            },
        };
