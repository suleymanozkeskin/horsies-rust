...
impl IsolatedTerminalizationTestDatabase {
    ...
    async fn create() -> Self {
        let database = Self::create_empty().await;
...
async fn assert_batch_snapshot_encoding(pool: &PgPool) {
...
async fn assert_terminal_result_digests(pool: &PgPool) {
    for admin in [false, true] {
        for payload in [None, Some("stored result".repeat(10000))] {
            let deferred = !admin && payload.is_some();
            let id = Uuid::new_v4();
            let workflow = Uuid::new_v4();
            seed_workflow(pool, &workflow.to_string(), "RUNNING").await;
            seed_task(
                pool,
                &id.to_string(),
                Seed {
                    is_workflow_task: deferred,
                    ..Seed::default()
                },
            )
            .await;
            if deferred {
                seed_wf_task(pool, &workflow.to_string(), &id.to_string(), "RUNNING").await;
            }
            sqlx::query("UPDATE horsies_tasks SET result = $2 WHERE id = $1")
                .bind(id)
                .bind(&payload)
                .execute(pool)
                .await
                .unwrap();
            let mut tx = pool.begin().await.unwrap();
            sqlx::query("SELECT horsies_move_task_to_history($1,$2,$3,NOW(),$4,NULL,NULL)")
                .bind(id)
                .bind(match admin {
                    true => "CANCELLED",
                    false => "COMPLETED",
                ...
                .bind(match admin {
                    true => "CANCEL_ADMIN",
                    false => "COMPLETE_LOCKED",
                })
                .bind(&payload)
                .execute(tx.as_mut())
                .await
                .unwrap();
            let (result, prior, digest): (Option<Vec<u8>>, Option<Vec<u8>>, Option<Vec<u8>>) = sqlx::query_as("SELECT result_payload, prior_result_payload, result_digest FROM horsies_task_history WHERE task_id=$1")
                .bind(id).fetch_one(tx.as_mut()).await.unwrap();
            let expected_payload = payload.as_ref().map(|v| v.as_bytes().to_vec());
            match admin {
                true => {
                    assert_eq!(prior, expected_payload);
            ...
            let expected_digest: Option<Vec<u8>> = sqlx::query_scalar("SELECT sha256($1::bytea)")
            ...
            if deferred {
                let deferred: Option<Vec<u8>> = sqlx::query_scalar(
                    "SELECT result_digest FROM horsies_workflow_phase2_pending WHERE task_id=$1",
                )
                .bind(id)
                .fetch_one(tx.as_mut())
                .await
                .unwrap();
                assert_eq!(deferred, expected_digest);
            }
            tx.rollback().await.unwrap();
            assert_eq!(post_image(pool, &id.to_string()).await.status, "RUNNING");
            cleanup(pool, &[&id.to_string()]).await;
            cleanup_workflow(pool, &workflow.to_string()).await;
        }
    }
}
